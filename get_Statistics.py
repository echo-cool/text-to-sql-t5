...
print(
    "Number of examples & {} & {} \\\\".format(
        len(train_nl_examples), len(dev_nl_examples)
    )
)
print(
    "Mean sentence length & {}& {} \\\\".format(
        np.mean(train_nl_len), np.mean(dev_nl_len)
    )
)
print(
    "Mean SQL query length & {}& {} \\\\".format(
        np.mean(train_sql_len), np.mean(dev_sql_len)
    )
)
print(
    "Vocabulary size (natural language)& {}& {} \\\\".format(
        len(set(" ".join(train_nl_examples).split())),
        len(set(" ".join(dev_nl_examples).split())),
    )
)
print(
    "Vocabulary size (SQL)& {}& {} \\\\".format(
        len(set(" ".join(train_sql_examples).split())),
        len(set(" ".join(dev_sql_examples).split())),
    )
)
